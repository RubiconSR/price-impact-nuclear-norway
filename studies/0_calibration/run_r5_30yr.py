"""
R5 simulation — Full 30-year run (1991-2020) for fair comparison with R3.

Changes from R3:
- storage_price: NO1=11.5, NO2=17.0, NO3=9.5, NO4=17.5, NO5=21.0
- Branch: identical to R3
"""
...
SQL_FILE = SCENARIO_DIR / 'results' / f'powergama_r5_{SIM_YEAR_START}_{SIM_YEAR_END}.sqlite'
...
def extract_prices():
    ...
    print('\n' + '=' * 80)
    print(f'R5 vs R3 ZONE PRICES [EUR/MWh] — 30-year average (1991-2020)')
    print('=' * 80)
    print(f'{"Zone":<8} {"R3":>8} {"R5":>8} {"R5-R3":>8} {"Hist":>8} {"R5-Hist":>10} {"R5-Hist%":>10}')
    print('-' * 65)
    ...
    for zone in no_zones:
        ...
        r3 = r3_prices[zone]
        h = hist_target[zone]
        print(f'{zone:<8} {r3:>8.1f} {avg:>8.2f} {avg-r3:>+8.2f} {h:>8.1f} {avg-h:>+10.2f} {(avg-h)/h*100:>+10.1f}%')

    r3_avg = np.mean(list(r3_prices.values()))
    r5_avg = np.mean(list(zone_prices.values()))
    h_avg = np.mean(list(hist_target.values()))
    print('-' * 65)
    print(f'{"Avg":<8} {r3_avg:>8.1f} {r5_avg:>8.2f} {r5_avg-r3_avg:>+8.2f} {h_avg:>8.1f} {r5_avg-h_avg:>+10.2f} {(r5_avg-h_avg)/h_avg*100:>+10.1f}%')
    ...
    # Isolate storage_price effect (R5 - R3)
    print(f'\n{"="*80}')
    print(f'ISOLERT EFFEKT AV STORAGE_PRICE-ENDRINGER (R5 - R3)')
    print(f'{"="*80}')
    print(f'{"Zone":<8} {"sp R3":>8} {"sp R5":>8} {"sp diff":>8} {"Pris R3":>10} {"Pris R5":>10} {"Prisdiff":>10}')
    print('-' * 65)
    sp_r3 = {'NO1': 13.0, 'NO2': 17.0, 'NO3': 9.5, 'NO4': 14.0, 'NO5': 17.0}
    sp_r5 = {'NO1': 11.5, 'NO2': 17.0, 'NO3': 9.5, 'NO4': 17.5, 'NO5': 21.0}
    for zone in no_zones:
        print(f'{zone:<8} {sp_r3[zone]:>8.1f} {sp_r5[zone]:>8.1f} {sp_r5[zone]-sp_r3[zone]:>+8.1f} {r3_prices[zone]:>10.1f} {zone_prices[zone]:>10.2f} {zone_prices[zone]-r3_prices[zone]:>+10.2f}')
...
if __name__ == '__main__':
    print('=' * 60)
    print('R5 CASE — 30-year simulation (1991-2020)')
    print('storage_price: NO1=11.5, NO2=17, NO3=9.5, NO4=17.5, NO5=21')
    print('=' * 60)
